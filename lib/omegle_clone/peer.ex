defmodule OmegleClone.Peer do
  ...
  def handle_info({:ex_webrtc, pc, {:ice_candidate, candidate}}, %{pc: pc} = state) do
    ...
    :ok = RoomChannel.send_candidate(state.channel, body)
  ...
  def handle_info({:ex_webrtc, pc, {:connection_state_change, :connected}}, %{pc: pc} = state) do
    ...
    :ok = Room.mark_ready(state.room_id, state.id)
  ...
  def terminate(_reason, state) do
    # Optionally clean up peer channel itself if it doesn't
    :ok = RoomChannel.close(state.channel)
  ...
  defp send_offer(%{pc: pc} = state) do
    ...
    :ok = RoomChannel.send_offer(state.channel, offer.sdp)
